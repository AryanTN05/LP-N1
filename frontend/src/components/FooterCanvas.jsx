...
export default function FooterCanvas() {
  ...
  useEffect(() => {
    ...
    // ── Surface particles (Fibonacci distribution) ────────────
    const ptCount = 2800;
    ...
    // ── Group all globe elements ──────────────────────────────
    const group = new THREE.Group();
    latLines.forEach(({ line }) => { scene.remove(line); group.add(line); });
    lngLines.forEach(({ line }) => { scene.remove(line); group.add(line); });
    scene.remove(globe); group.add(globe);
    scene.remove(wake);  group.add(wake);
    scene.add(group);
